...
contains 
  ...
  subroutine hgproject(proj_type,mla,unew,uold,rhohalf,p,gp,dx,dt,t,&
                       the_bc_tower, press_comp,divu_rhs,div_coeff_1d,&
                       div_coeff_3d,eps_in)
    ...
    subroutine hg_update(mla,proj_type,unew,uold,gp,gphi,rhohalf,p,phi,ng,dt,the_bc_level)
      ...
      do n = nlevs, 2, -1
         ...
         call multifab_fill_ghost_cells(unew(n),unew(n-1),ng,mla%mba%rr(n-1,:), &
                                        the_bc_level(n-1),the_bc_level(n),1,1,dm,dx(n-1:n,:),t)
      end do
