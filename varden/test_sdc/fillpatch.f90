...
contains
  ...
  subroutine fillpatch(fine, crse, ng, ir, bc_crse, bc_fine, icomp_fine, icomp_crse, &
                       bcomp, nc,dx,t,no_final_physbc_input, lim_slope_input,&
                       lin_limit_input)
    ...
    ! passed in dx(n:n-1,:)
    call multifab_physbc(crse,icomp_crse,bcomp,nc,bc_crse,dx(1,:),t)
    !
    ...
    if(.not. no_final_physbc) then
       call multifab_physbc(fine, icomp_fine, bcomp, nc, bc_fine,dx(2,:),t)
    end if
