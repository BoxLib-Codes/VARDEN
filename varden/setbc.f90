...
!     Local variables
      ...
      if (visc_coef > ZERO) then
         ...
         if (bc(1,1) .eq. WALL) vel(lo(1)-ng:lo(1)- 1,lo(2)-ng:hi(2)+ng,1:2) = ZERO
         if (bc(1,2) .eq. WALL) vel(hi(1)+ 1:hi(1)+ng,lo(2)-ng:hi(2)+ng,1:2) = ZERO
         if (bc(2,1) .eq. WALL) vel(lo(1)-ng:hi(1)+ng,lo(2)-ng:lo(2)- 1,1:2) = ZERO
         if (bc(2,2) .eq. WALL) vel(lo(1)-ng:hi(1)+ng,hi(2)+ 1:hi(2)+ng,1:2) = ZERO
...
!     Local variables
      ...
      if (bc(1,1) .eq. WALL) then
            ...
            do k = lo(3)-1,hi(3)+1
            do j = lo(2)-1,hi(2)+1
               vel(lo(1)-ng:lo(1)-1,j,k,1) = VX_INLET
               vel(lo(1)-ng:lo(1)-1,j,k,2) = VY_INLET
               vel(lo(1)-ng:lo(1)-1,j,k,3) = VZ_INLET
            end do
            end do
      end if
      ...
      if (bc(1,2) .eq. WALL) then
            ...
            do k = lo(3)-1,hi(3)+1
            do j = lo(2)-1,hi(2)+1
               vel(hi(1)+1:hi(1)+ng,j,k,1) = VX_INLET
               vel(hi(1)+1:hi(1)+ng,j,k,2) = VY_INLET
               vel(hi(1)+1:hi(1)+ng,j,k,3) = VZ_INLET
            end do
            end do
      end if
      ...
      if (bc(2,1) .eq. WALL) then
            ...
            do k = lo(3)-1,hi(3)+1
            do i = lo(1)-ng,hi(1)+ng
               vel(i,lo(2)-ng:lo(2)-1,k,1) = VX_INLET
               vel(i,lo(2)-ng:lo(2)-1,k,2) = VY_INLET
               vel(i,lo(2)-ng:lo(2)-1,k,3) = VZ_INLET
            end do
            end do
      end if
      ...
      if (bc(2,2) .eq. WALL) then
            ...
            do k = lo(3)-1,hi(3)+1
            do i = lo(1)-ng,hi(1)+ng
               vel(i,hi(2)+1:hi(2)+ng,k,1) = VX_INLET
               vel(i,hi(2)+1:hi(2)+ng,k,2) = VY_INLET
               vel(i,hi(2)+1:hi(2)+ng,k,3) = VZ_INLET
            end do
            end do
      end if
      ...
      else if (bc(3,1) .eq. INLET) then
            do j = lo(2)-ng,hi(2)+ng
            do i = lo(1)-ng,hi(1)+ng
               vel(i,j,lo(3)-ng:lo(3)-1,1) = VX_INLET
               vel(i,j,lo(3)-ng:lo(3)-1,2) = VY_INLET
               vel(i,j,lo(3)-ng:lo(3)-1,3) = VZ_INLET
            end do
            end do
      end if
      ...
      else if (bc(3,2) .eq. INLET) then
            do j = lo(2)-ng,hi(2)+ng
            do i = lo(1)-ng,hi(1)+ng
               vel(i,j,hi(3)+1:hi(3)+ng,2) = VX_INLET
               vel(i,j,hi(3)+1:hi(3)+ng,2) = VY_INLET
               vel(i,j,hi(3)+1:hi(3)+ng,3) = VZ_INLET
            end do
            end do
      end if
      ...
      if (visc_coef > ZERO) then
         ...
         if (bc(1,1) .eq. WALL) vel(lo(1)-ng:lo(1)- 1,lo(2)-ng:hi(2)+ng,lo(3)-ng:hi(3)+ng,1:3) = ZERO
         if (bc(1,2) .eq. WALL) vel(hi(1)+ 1:hi(1)+ng,lo(2)-ng:hi(2)+ng,lo(3)-ng:hi(3)+ng,1:3) = ZERO
         if (bc(2,1) .eq. WALL) vel(lo(1)-ng:hi(1)+ng,lo(2)-ng:lo(2)- 1,lo(3)-ng:hi(3)+ng,1:3) = ZERO
         if (bc(2,2) .eq. WALL) vel(lo(1)-ng:hi(1)+ng,hi(2)+ 1:hi(2)+ng,lo(3)-ng:hi(3)+ng,1:3) = ZERO
         if (bc(3,1) .eq. WALL) vel(lo(1)-ng:hi(1)+ng,lo(2)-ng:hi(2)+ng,lo(3)-ng:lo(3)- 1,1:3) = ZERO
         if (bc(3,2) .eq. WALL) vel(lo(1)-ng:hi(1)+ng,lo(2)-ng:hi(2)+ng,hi(3)+ 1:hi(3)+ng,1:3) = ZERO
...
!              + 3.0_dp_t * s(lo(1)+2,j,n) ) * EIGHTH
         ...
         n = 1
         do j = lo(2)-1,hi(2)+1
            s(lo(1)-ng:lo(1)-1,j,n) = RHO_INLET
         end do
         do n = 2,ncomp
         do j = lo(2)-1,hi(2)+1
            s(lo(1)-ng:lo(1)-1,j,n) = s(lo(1),j,n)
...
!              + 3.0_dp_t * s(hi(1)-2,j,n) ) * EIGHTH
         ...
         n = 1
         do j = lo(2)-1,hi(2)+1
            s(hi(1)+1:hi(1)+ng,j,n) = RHO_INLET
         end do
         do n = 2,ncomp
         do j = lo(2)-1,hi(2)+1
            s(hi(1)+1:hi(1)+ng,j,n) = s(hi(1),j,n)
...
!              + 3.0_dp_t * s(i,lo(2)+2,n) ) * EIGHTH
         ...
         n = 1
         do i = lo(1)-ng,hi(1)+ng
            s(i,lo(2)-ng:lo(2)-1,n) = RHO_INLET
         end do
         do n = 2,ncomp
         do i = lo(1)-ng,hi(1)+ng
...
!              + 3.0_dp_t * s(i,hi(2)-2,n) ) * EIGHTH
         ...
         n = 1
         do i = lo(1)-ng,hi(1)+ng
            s(i,hi(2)+1:hi(2)+ng,n) = RHO_INLET
         end do
         do n = 2,ncomp
         do i = lo(1)-ng,hi(1)+ng
...
!     Local variables
      ...
      if (bc(1,1) .eq. WALL) then
         ...
         do k = lo(3)-1,hi(3)+1
         do j = lo(2)-1,hi(2)+1
            s(lo(1)-ng:lo(1)-1,j,k,n) = RHO_INLET
         end do
         end do
         do n = 2,ncomp
         do k = lo(3)-1,hi(3)+1
      ...
      if (bc(1,2) .eq. WALL) then
         ...
         do k = lo(3)-1,hi(3)+1
         do j = lo(2)-1,hi(2)+1
            s(hi(1)+1:hi(1)+ng,j,k,n) = RHO_INLET
         end do
         end do
         do n = 2,ncomp
         do k = lo(3)-1,hi(3)+1
      ...
      if (bc(2,1) .eq. WALL) then
         ...
         do k = lo(3)-1,hi(3)+1
         do i = lo(1)-ng,hi(1)+ng
            s(i,lo(2)-ng:lo(2)-1,k,n) = RHO_INLET
         end do
         end do
         do n = 2,ncomp
         do k = lo(3)-1,hi(3)+1
      ...
      if (bc(2,2) .eq. WALL) then
         ...
         do k = lo(3)-1,hi(3)+1
         do i = lo(1)-ng,hi(1)+ng
            s(i,hi(2)+1:hi(2)+ng,k,n) = RHO_INLET
         end do
         end do
         do n = 2,ncomp
         do k = lo(3)-1,hi(3)+1
      ...
      if (bc(3,1) .eq. WALL) then
         ...
         do j = lo(2)-ng,hi(2)+ng
         do i = lo(1)-ng,hi(1)+ng
            s(i,j,lo(3)-ng:lo(3)-1,n) = RHO_INLET
         end do
         end do
         do n = 2,ncomp
         do j = lo(2)-ng,hi(2)+ng
      ...
      if (bc(3,2) .eq. WALL) then
         ...
         do j = lo(2)-ng,hi(2)+ng
         do i = lo(1)-ng,hi(1)+ng
            s(i,j,hi(3)+1:hi(3)+ng,n) = RHO_INLET
         end do
         end do
         do n = 2,ncomp
         do j = lo(2)-ng,hi(2)+ng
