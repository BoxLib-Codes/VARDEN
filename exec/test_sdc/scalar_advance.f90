...
contains
  ...
  subroutine scalar_advance(mla,uold,sold,snew,umac, &
    ...
    ! NOTE: even when using mass fractions, rho*y_i is stored in s
    ...
    if (diff_coef .gt. ZERO) then
       do comp = 2, nscal
          bc_comp = dm+comp
          call get_explicit_diffusive_term(mla,laps,sold,comp,bc_comp,&
                                           dx,the_bc_tower,is_vel=.false.)
       enddo
    else
      do n = 1, nlevs
    ...
    !***********************************
    call update(mla,sold,umac,sedge,sflux,scal_force,snew,adv_s,dx,&
                dt,t,is_vel,is_conservative,the_bc_tower%bc_tower_array)
